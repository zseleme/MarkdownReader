...
async function handleSave(request, env) {
    ...
    try {
        ...
        const metadata = { id, slug, title, created: new Date().toISOString(), size: body.content.length };
        ...
        console.error('Save error:', error.message, '- IP:', ip);
        return new Response(
...
async function handleLoad(request, env) {
    ...
    try {
        ...
        console.error('Load error:', error.message, '- ID:', id);
        return new Response(
            JSON.stringify({ success: false, error: 'Internal server error' }),
            { status: 500, headers }
        );
...
export default {
    async fetch(request, env) {
        ...
        if (pathname === '/api/save') {
            if (method === 'OPTIONS') return new Response(null, { status: 200, headers: corsHeaders(origin, 'POST, OPTIONS') });
            if (method === 'POST') return handleSave(request, env);
            return new Response(JSON.stringify({ success: false, error: 'Method not allowed' }), { status: 405, headers: { 'Content-Type': 'application/json' } });
        }

        if (pathname === '/api/load') {
            if (method === 'OPTIONS') return new Response(null, { status: 200, headers: corsHeaders(origin, 'GET, OPTIONS') });
            if (method === 'GET') return handleLoad(request, env);
            return new Response(JSON.stringify({ success: false, error: 'Method not allowed' }), { status: 405, headers: { 'Content-Type': 'application/json' } });
        }
